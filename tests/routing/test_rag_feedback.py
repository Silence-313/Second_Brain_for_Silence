...
class TestRagFeedback:
    def test_record_retrieval(self) -> None:
        rag = RagFeedback()
        rag.record_retrieval(RetrievalRecord(query="test", retrieved_docs=["a.md", "b.md"], used_docs=["a.md"], answer_quality=0.8))
    ...
    def test_serialize_deserialize(self) -> None:
        rag = RagFeedback()
        rag.record_retrieval(RetrievalRecord(query="q", retrieved_docs=["a.md"], used_docs=["a.md"], answer_quality=0.7))
    ...
    def test_get_negative_signals(self) -> None:
        rag = RagFeedback()
        for i in range(5):
            rag.record_retrieval(RetrievalRecord(query=f"q{i}", retrieved_docs=["doc.md"], used_docs=[], answer_quality=0.3))
        signals = rag.get_negative_signals()
        assert isinstance(signals, list)

    def test_get_cluster_success_rate(self) -> None:
        rag = RagFeedback()
        rag.record_retrieval(RetrievalRecord(query="what is python", retrieved_docs=["a.md"], used_docs=["a.md"], answer_quality=0.8))
